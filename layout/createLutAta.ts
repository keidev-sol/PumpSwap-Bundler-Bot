...
export const createAndSendV0Tx = async (txInstructions: TransactionInstruction[]) => {
    ...
    console.log("create LUT tx simulate =>:", await connection.simulateTransaction(transaction))
...
//         console.log("🚀 ~ createLUT ~ slot:", slot)
...
export const create_extend_lut_ata = async () => {
    ...
    try {
        ...
        // const lookupTableAddress = await createLUT(SIGNER_WALLET)
        ...
        await addAddressesToTable(new PublicKey("EXFhCDLDQWh7QLLDPXJDL291wNeBXkVPQCU2FTEnCkSE"), mint)
        await outputBalance(SIGNER_WALLET.publicKey)
